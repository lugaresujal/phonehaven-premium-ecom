...
import { Loader2, AlertCircle, CheckCircle, Mail, Eye, EyeOff, KeyRound } from "lucide-react";
...
export const Route = createFileRoute("/forgot-password")({
  head: () => ({
    meta: [
      { title: "Forgot Password — House of Phones" },
      { name: "description", content: "Reset your House of Phones account password." },
...
function ForgotPasswordPage() {
  ...
  const onRequestToken = async (e: React.FormEvent) => {
    e.preventDefault();
      ...
      <section className="container-hop py-10 md:py-16 max-w-lg mx-auto">
        <div className="bg-card border border-border rounded-3xl p-6 sm:p-10">
          <div className="flex items-center gap-3 mb-2">
            {step === "request" && <Mail size={24} className="text-primary" />}
            {step === "reset" && <KeyRound size={24} className="text-primary" />}
            {step === "done" && <CheckCircle size={24} className="text-green-600" />}
            <h2 className="font-serif text-2xl">
              {step === "request" && "Forgot Password"}
              {step === "reset" && "Verify Code & Reset"}
              {step === "done" && "Password Reset Complete"}
            </h2>
          </div>
          <p className="text-sm text-muted-foreground mt-1">
            {step === "request" && "Enter your registered email address to receive a 6-digit verification code."}
            {step === "reset" && `We sent a 6-digit verification code to ${email}. Enter it below along with your new password.`}
          ...
          {step === "done" && (
            <div className="mt-6 flex items-start gap-3 rounded-2xl border border-green-500/30 bg-green-500/5 p-5 text-sm text-green-700">
              ...
              <div>
                ...
                <p className="mt-1 text-xs text-muted-foreground">Your account is ready to use with your new credentials.</p>
          ...
          {step === "request" && (
            <form className="mt-6 space-y-4" onSubmit={onRequestToken} noValidate>
              {error && (
              ...
              <div>
                <label className="block text-xs uppercase tracking-wider text-muted-foreground mb-1.5">Registered Email Address</label>
                <input
                  type="email"
                  value={email}
                  ...
                  autoComplete="email"
                  inputMode="email"
                  className={inputCls(error ? "bad" : undefined)}
                />
              </div>
              <button
                type="submit"
          ...
          {step === "reset" && (
            <form className="mt-6 space-y-4" onSubmit={onResetPassword} noValidate>
              {error && (
                <div role="alert" className="flex items-start gap-2 rounded-xl border border-destructive/30 bg-destructive/5 px-4 py-3 text-sm text-destructive">
                  <AlertCircle size={16} className="mt-0.5 shrink-0" />
                  <span>{error}</span>
